...
class CharityData(DBMaterializedView):
    id = models.IntegerField(primary_key=True, db_index=True)
    source = models.CharField(
        max_length=255,
        choices=DataSources.choices,
        default=DataSources.CCEW,
        db_index=True,
    )
...
cl AS (
    SELECT
        organisation_number,
        array_agg(classification_code::text) FILTER (
            WHERE
...
social AS (
    SELECT
        ...
        count(social_base.social->>'handle') FILTER (WHERE (social_base.social->>'platform') = 'twitter') AS has_twitter,
        count(social_base.social->>'handle') FILTER (WHERE (social_base.social->>'platform') = 'facebook') AS has_facebook
    FROM
        social_base
    WHERE
...
grants AS (
    SELECT
        ...
        array_agg(
            DISTINCT g."fundingOrganization_name"
        ) AS funders_list
    FROM
...
geo AS (
    SELECT
        ...
        g.rgn as region_code,
        g.ctry as country_code,
        g.laua as laua_code,
        g.lat as latitude,
        g.long as longitude
    FROM
...
SELECT
    ...
    areas.areas_list AS areas_list,
	cl.causes AS causes_list,
	cl.beneficiaries AS beneficiaries_list,
	cl.operations AS operations_list,
	trustees.trustees_list AS trustees_list,
	grants.funders_list AS funders_list,
	cc.date_of_registration AS registration_date,
    social.has_twitter > 0 AS social_twitter,
    social.has_facebook > 0 AS social_facebook,
    geo.region_code AS region_code,
    geo.country_code AS country_code,
    geo.laua_code AS laua_code
FROM
